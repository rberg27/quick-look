...
class StencilGenerator(nn.Module):
    """Generator for inpainting. Fully Generates the image and then applies the mask over the full image."""
    def __init__(self, latent_dim=256, channels=64):
        super().__init__()
    ...
    def forward(self, z, mask, masked_image):
        '''
        z: latent vector
...
class StencilGAN(pl.LightningModule):
    """GAN that tries to generate an image, a mask of that image is applied on top of what's generated, and the original image is used as a guide."""
    def __init__(
        self,
        latent_dim=256,
        channels=64,
        ...
        beta2=0.999,
        lambda_real=1.0,
        lambda_reconstruction=0.1,  # Weight for optional reconstruction loss
        use_reconstruction_loss=True,  # Whether to use reconstruction loss
        weights_dir=config.STENCIL_GAN_MODEL_WEIGHTS_DIR  # Directory to save weights
    ):
        super().__init__()
        ...
        # GAN components
        self.generator = StencilGenerator(latent_dim=latent_dim, channels=channels)
    ...
    def forward(self, masked_images, masks):
        ...
        # The generator outputs the full image (masked area filled in)
        generated_full_images = self.generator(latent, masks, masked_images)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Generate full images (with masked area filled in)
        ...
        # Real image similarity loss - compare the generated full image to the original
        g_real_loss = 0
        if self.lambda_real > 0:
            g_real_loss = self.reconstruction_loss(original_images, generated_full_images)
        
        # Reconstruction loss (optional) - focus on the masked area
        g_rec_loss = 0
        ...
        if self.lambda_real > 0:
            self.log('g_real_loss', g_real_loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.lambda_real > 0:
            self.log('val_g_real_loss', g_real_loss)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train StencilGAN on BYO GAN dataset')
    ...
    parser.add_argument('--output_dir', type=str, default='./stencil_gan_results', help='Directory to save results')
    ...
    parser.add_argument('--loaded_model_name', type=str, default=None, help='Name of loaded model')
    ...
    # Create StencilGAN model
    print("Creating StencilGAN model...")
    model = StencilGAN(
